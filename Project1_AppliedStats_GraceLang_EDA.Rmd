...
par(mfrow=c(2,2))
plot(full.model) ## There may be some outliers needing to be removed, figure out how to isolate the cooks D graph
# leveragePlots(full.model) - investigating leverage points by parameter
# avPlots(full.model)
...
#dim(Train_Life) #92   20
#dim(Test_Life) #91   20

reg.fwd=regsubsets(Life.expectancy~.,data=Train_Life,method="forward",nvmax=20)
...
for (i in 1:19){
  ...
  testASE[i]<-mean((Life14$Life.expectancy-predictions)^2)
}
par(mfrow=c(1,1))
plot(1:92,testASE,type="l",xlab="# of predictors",ylab="test vs train ASE",ylim=c(0.3,0.8))
index<-which(testASE==min(testASE))
points(index,testASE[index],col="red",pch=10)
rss<-summary(reg.fwd)$rss
lines(1:92,rss/100,lty=3,col="blue")  
```
