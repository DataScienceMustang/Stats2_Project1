...
trial = trial3 %>% mutate(ResidualSq7 = (Predictions - Actual)^2)
...
```{r}
# with all variables included it looks like the best chance I have is with k = 22. But that minimum is still far greater than the LM's. need to look at restricting variables. With 4 variables it looks like I can get it to sub 4 (~3.7).
...
{
rmseknn = data.frame(rmse = numeric(90), k = numeric(90))
index<-sample(1:dim(Life14)[1],128,replace=F)
train<-Life14[index,]
test<-Life14[-index,]
for(i in 1:numks)
{
  predknn <- knn.reg(train[, knnvector1], test[, knnvector1], train$Life.expectancy, k = i)
...
plot(seq(1,numks,1),MeanRMSE, type = "l")
```
