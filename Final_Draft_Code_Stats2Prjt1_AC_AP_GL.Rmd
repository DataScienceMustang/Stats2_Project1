...
# It would be helpful if we drop Country and Year here -  is there a reason we cant? It looks like everything below this has some lines dedicated to taking out country and year, could save ourselves some room?

# If we cant take it out here - I'll move my scale variables to be created later, I cant have any non-numerics for knn.reg and right now Life14 is passing Country and Year
...
cor.xy <- cor(Life14 %>% dplyr::select(-c(Country, Year, Status)), use = "complete.obs")
...
# Gut Model - EDA first - human variable selection - Some tech use to look at optimum number of predictors - income comp increases model variance
```{r}
...
# complex model - interactions - higher order terms - autoselection methods - through many iterations I found no indication significant interactions, however 
while playing with some models I found an advantageous result when using Schoolfactor:scale(percentage.expeniture). The interaction is not significant but provides a slight edge in prediction (< 2% for most runs)
```{r}
modeltest2 <-  lm(Life.expectancy ~ Income.composition.of.resources + SchoolFactor + LHIV.AIDS + Adult.Mortality + Linfant.deaths + 
...
This model is here to run against the knn for comparison. It uses the same scaled variables as the knnvector3
```{r}
...
# Getting results of the 2 Models
```{r}
...
d3 = data.frame(R2 = R2(predictions3,testS$Life.expectancy),RMSE = RMSE(predictions3,testS$Life.expectancy), MAE = MAE(predictions3, testS$Life.expectancy))
...
PredAll %>% ggplot(aes(x = Value, y = ActualValue, fill = Model)) + geom_point(aes(color = Model)) + geom_smooth(formula = y~x)+
  facet_wrap(facets = PredAll$Model) + ggtitle("Visual Model Fit") + xlab("Predicted Life Expectancy") + ylab("Actual Life Expectancy")

#Barchart of results
PredAll  %>% mutate(difference = (Value - ActualValue)) %>% group_by(Model) %>% 
  summarise(RMSE = sqrt(sum(difference^2)/nrow(test))) %>%
  ggplot(aes(x = reorder(Model,-RMSE), y = RMSE)) + geom_col(width = 0.75, aes(fill = Model)) + 
...
Model.Average.RMSE = cbind(rmse1, rmse2)
...
# Here we can see that knnvector2 is significantly different that knnvector1 and knnvector3. 1 and 3 are not significantly different
# Dont forget multiple comparison adjustment
knnrmsedf1 %>% ggplot(aes(x = variable, y = value)) + geom_boxplot()
